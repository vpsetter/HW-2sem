...
void Visualizer_Julia::initialize()
{
...
void Visualizer_Julia::handle_key_pressed(sf::Keyboard::Key code)
{
...
void Visualizer_Julia::render()
{
	...
	ss <<
		...
		"Function: f(z) = z * z + c, c = " << std::fixed << std::setprecision(2) <<
		c.real() << (c.imag() < 0.0 ? " - " : " + ") << std::abs(c.imag()) << "i";
