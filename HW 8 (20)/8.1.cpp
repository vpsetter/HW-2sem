...
struct count_points
{
	void operator()(const std::size_t N, F predicate, Distr_t distribution, std::atomic<std::size_t> &counter)
	{
...
std::size_t parallel_count(const std::size_t points_num, F predicate, Distr_t distribution)
{
	...
	{
		std::packaged_task < void(const std::size_t, F, Distr_t, std::atomic<std::size_t>&) > task{ count_points <F, Distr_t>() };

		futures[i] = task.get_future();
		threads[i] = std::thread(std::move(task), block_size, predicate, distribution, std::ref(counter));
	}

	count_points < F, Distr_t >()(points_num - (num_threads - 1U) * block_size, predicate, distribution, counter);
